...
@app.route("/")
def home():
    return render_template('login.html', hide_verification=True)
...
def login():
    ...
    try:
        ...
        return render_template("login.html", login_message="Invalid Credentials", hide_verification=True)
...
def list_by_image_data():
    if request.method == "POST":
        ...
        if len(links) <= 0:
            list_by_image_data_message = "No Matching Image Found!"
        else:
            for item in response.json()['links']:
                list_by_image_data_message += item
                list_by_image_data_message += '\n'
